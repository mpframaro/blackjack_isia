...
class CustomBlackjackEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action:  # hit: add a card to players hand and return
            ...
            #1 se jogador ganha, -1 se dealer perde
            ...
            #penalizar o jogador se termina o jogo com uma soma baixa
            if sum_hand(self.player) < 12:
                reward -=0.5

            #beneficiar jogador com mão entre 18-20 (zona muito boa)
            elif 18 <= sum_hand(self.player) <= 20:
                reward += 0.5

            #penalizar jogador com mão superior a 20 (muito perigoso)
            elif sum_hand(self.player) == 21:
                reward -= 0.5

            #penalizar demasiados "hits" numa só jogada
